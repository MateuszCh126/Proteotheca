      ...
      <form onSubmit={submit} className="glass-panel w-full max-w-md space-y-4 border border-cyan-400/20 p-5">
        <div className="flex items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-2 text-sm font-bold text-white">
            ...
            <span className="truncate">{activeMode === 'register' ? 'Create research account' : 'Sign in'}</span>
          </div>
          <button type="button" onClick={onClose} className="rounded-lg p-1.5 hover:bg-white/10" aria-label="Close auth dialog">
            <X className="h-4 w-4" />
          </button>
        </div>
