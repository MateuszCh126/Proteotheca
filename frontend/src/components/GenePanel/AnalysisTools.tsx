    ...
    <div className="space-y-4" data-testid="analysis-tools-panel">
      ...
      <div className="p-3 bg-white/5 border border-white/5 rounded-xl space-y-3">
        {activeTool === 'blast' && (
          <div className="space-y-2">
            ...
            <textarea
              ...
              className="w-full h-16 bg-black/40 text-3xs font-mono p-2 rounded border border-white/5 text-slate-400 focus:outline-none resize-none"
            />
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-3xs uppercase tracking-wider text-slate-400 font-bold block mb-1">Target Database</label>
                <div className="relative">
                  <select
                    value={blastDb}
                    onChange={(e) => setBlastDb(e.target.value)}
                    className="w-full bg-slate-900 border border-white/10 text-slate-200 rounded-lg px-2.5 py-1 text-2xs outline-none appearance-none"
                  >
              ...
              <div>
                <label className="text-3xs uppercase tracking-wider text-slate-400 font-bold block mb-1">E-Value Threshold</label>
                <div className="relative">
                  <select
                    value={eValue}
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2 text-2xs">
              {['human', 'mouse', 'rat', 'zebrafish'].map((species) => {
                ...
                return (
                  <label key={species} className="flex items-center space-x-2 p-1.5 bg-slate-900/50 rounded border border-white/5 cursor-pointer hover:bg-slate-900">
                    <input
                      type="checkbox"
                      checked={isChecked}
                      onChange={() => {
                    ...
                    />
                    <span className="capitalize text-slate-300 font-medium">{species} ({geneData.symbol})</span>
                  </label>
                );
              })}
            </div>
        ...
        {activeTool === 'foldseek' && (
          <div className="space-y-3">
            ...
            <div>
              <label className="text-3xs uppercase tracking-wider text-slate-400 font-bold block mb-1">Structural database</label>
              <div className="relative">
                <select
                  value={foldseekDb}
        ...
        <button
          ...
          className="w-full py-2 bg-gradient-to-r from-cyan-500 to-indigo-500 hover:from-cyan-400 hover:to-indigo-400 disabled:from-cyan-800 disabled:to-indigo-900 text-slate-950 text-2xs font-extrabold uppercase rounded-lg transition-all flex items-center justify-center gap-1.5 shadow-md shadow-cyan-500/10"
        >
      ...
      {isRunning && (
        <div className="space-y-1.5">
          <div className="flex justify-between text-3xs font-mono text-cyan-400 font-bold">
            <span>Executing search algorithm...</span>
            <span>{progress}%</span>
          </div>
          <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden">
            <div className="h-full bg-cyan-400 rounded-full transition-all duration-200" style={{ width: `${progress}%` }} />
          </div>
        </div>
      )}
      ...
      {completed && !isRunning && (
        <div className="space-y-3 animate-fade-in">
          <div className="flex items-center gap-1.5 text-xs text-emerald-400 font-bold font-outfit">
          ...
          {activeTool === 'blast' && (
            <div className="space-y-2">
              ...
              <div className="space-y-1.5 max-h-48 overflow-y-auto custom-scrollbar pr-1">
                {blastHits.map((hit) => (
                  <div key={hit.id} className="p-2 bg-white/5 rounded-lg border border-white/5 flex items-center justify-between text-2xs hover:border-cyan-500/10 transition-all">
                    <div>
                      <span className="font-bold text-white block">{hit.id}</span>
                      <span className="text-3xs text-slate-500 block truncate max-w-xs">{hit.desc}</span>
                    </div>
                    <div className="text-right font-mono">
                      <span className="text-cyan-400 font-bold block">Id: {hit.identity.toFixed(1)}%</span>
                      <span className="text-3xs text-slate-500 block">E: {hit.evalue} | S: {hit.score}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {activeTool === 'msa' && (
            <div className="space-y-2.5">
              ...
              <div className="p-2.5 bg-black/40 rounded-lg border border-white/5 font-mono text-3xs space-y-1.5 overflow-x-auto">
                {msaSequences.map((seq) => (
                  <div key={seq.species} className="flex whitespace-nowrap">
                    <span className="w-16 font-bold text-slate-400 shrink-0">{seq.species}:</span>
                    <span className="text-slate-200 tracking-wider">
                      {seq.seq.split('-').map((char, i) => {
                        let color = 'text-slate-400';
                        if ('AVFIPLM'.includes(char)) color = 'text-blue-400'; // hydrophobic
                        if ('RK'.includes(char)) color = 'text-red-400';       // basic
                        if ('DE'.includes(char)) color = 'text-purple-400';    // acidic
                        if ('STNQYHCW'.includes(char)) color = 'text-green-400'; // polar/charged
                ...
                <div className="flex whitespace-nowrap">
                  <span className="w-16 font-bold text-cyan-400 shrink-0">Consensus:</span>
                  <span className="text-cyan-400 tracking-wider font-extrabold">{msaConsensus}</span>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1.5 max-h-48 overflow-y-auto custom-scrollbar pr-1">
                {foldseekHits.map((hit) => (
                  <div key={hit.pdbId} className="p-2 bg-white/5 rounded-lg border border-white/5 flex items-center justify-between text-2xs hover:border-cyan-500/10 transition-all">
                    <div>
                      <div className="flex items-center gap-1.5">
                        <span className="font-bold text-white font-outfit">{hit.pdbId}</span>
                        <span className="text-3xs font-mono bg-white/10 text-slate-400 px-1 rounded">Chain {hit.chain}</span>
                      </div>
                      <span className="text-3xs text-slate-500 block truncate max-w-xs">{hit.title}</span>
                    </div>
                    <div className="text-right font-mono">
                      <span className="text-cyan-400 font-bold block">TM-Score: {hit.tmScore.toFixed(3)}</span>
                      <span className="text-3xs text-slate-500 block">RMSD: {hit.rmsd.toFixed(1)} Å | Res: {hit.alnRes}</span>
                    </div>
                  </div>
                ))}
              </div>
