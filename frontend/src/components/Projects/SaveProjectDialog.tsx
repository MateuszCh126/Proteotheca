...
export default function SaveProjectDialog({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-surface/75 px-4 backdrop-blur-sm">
      <form onSubmit={submit} className="glass-panel w-full max-w-lg space-y-4 border border-line p-5">
        <div className="flex items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-2 text-sm font-bold text-ink">
            <Save className="h-4 w-4 shrink-0 text-ink" />
            <span className="truncate">{t('projects.saveResearchProject')}</span>
          </div>
          <button type="button" onClick={onClose} className="rounded-lg p-1.5 hover:bg-wash" aria-label={t('projects.closeSaveDialog')}>
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="flex items-center justify-between gap-3 text-[0.65rem] text-ink-2">
        ...
        <div className="flex gap-3">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 rounded-lg border border-line px-3 py-2 text-sm text-ink-2 transition hover:bg-wash hover:text-ink"
          >
            Cancel
          </button>
          <button
            type="submit"
            disabled={saving}
            className="flex-1 rounded-lg bg-ink px-3 py-2 text-sm font-bold text-paper transition hover:bg-ink disabled:opacity-60"
          >
