...
export const GlobalFreqGauge: React.FC<{ frequency: number }> = ({ frequency }) => {
  ...
  let color = 'hsl(222, 20%, 22%)'; // Ultra-rare (Cyan)
  if (frequency >= 0.01) {
    color = 'hsl(220, 9%, 55%)';  // Common (Amber/Red)
  } else if (frequency >= 0.0001) {
    color = 'hsl(220, 10%, 72%)';  // Rare (Green)
  }
  ...
  return (
    <div className="flex flex-col items-center justify-center relative w-full h-36" data-testid="global-af-gauge">
      ...
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/4 text-center">
        <span className="text-3xs text-ink-2 font-bold uppercase tracking-wider block">{t('variant.globalAf')}</span>
        <span className="text-base font-extrabold tracking-tight text-ink block font-sans" data-testid="global-af-value">
          {frequency.toExponential(3)}
        </span>
        <span className="text-3xs text-ink-3 font-mono block mt-0.5">
...
export const PopulationBarChart: React.FC<{ populations: GnomadPopulation[] }> = ({ populations }) => {
  ...
  const getBarColor = (freq: number) => {
    if (freq >= 0.01) return 'hsl(220, 9%, 55%)';      // Common (Amber/Red)
    if (freq >= 0.0001) return 'hsl(220, 10%, 72%)';    // Rare (Green)
    return 'hsl(222, 20%, 22%)';                       // Ultra-rare (Cyan)
  };
      ...
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          <XAxis 
            ...
            stroke="rgba(20,24,34,0.10)"
            tick={{ fill: 'hsl(220, 9%, 45%)', fontSize: 9, fontFamily: 'monospace' }}
            domain={[0, 'dataMax']}
          />
          <YAxis 
            dataKey="pop" 
            type="category" 
            stroke="rgba(20,24,34,0.10)"
            tick={{ fill: 'hsl(220, 9%, 40%)', fontSize: 9, fontFamily: 'var(--font-sans)' }}
            width={85}
          />
          <Tooltip
            cursor={{ fill: 'rgba(20,24,34,0.04)' }}
            content={({ active, payload }) => {
              if (active && payload && payload.length) {
                const item = payload[0].payload as GnomadPopulation;
                return (
                  <div className="glass-panel p-2.5 rounded-lg border border-line shadow-xl bg-surface/90 backdrop-blur-md text-3xs">
                    <p className="font-bold text-ink mb-1 font-sans">{item.pop}</p>
                    <p className="text-ink-2">
                      {t('variant.freq')} <span className="font-mono text-ink">{item.freq.toExponential(4)}</span>
                    </p>
                    <p className="text-ink-2 text-2xs mt-0.5">
...
export const AlleleFreqChart: React.FC<AlleleFreqChartProps> = ({ data }) => {
  ...
  return (
    <div className="space-y-4">
      <div>
        <span className="text-3xs uppercase tracking-wider text-ink-2 font-bold block mb-2">
